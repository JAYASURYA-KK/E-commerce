...
if(isset($_POST['update_location'])) {
    $location_code = mysqli_real_escape_string($db, trim($_POST['location_code']));
    $order_id = mysqli_real_escape_string($db, $_GET['user_upd']);
    
    // If location code is empty, set it to NULL, otherwise use the provided value
    if(empty($location_code)) {
        $update_sql = "UPDATE users_orders SET location_code = NULL WHERE o_id = '$order_id'";
    } else {
        $update_sql = "UPDATE users_orders SET location_code = '$location_code' WHERE o_id = '$order_id'";
    }
    
    $update_result = mysqli_query($db, $update_sql);
    
    if($update_result) {
        echo "<script>alert('Location code updated successfully!'); window.location.reload();</script>";
    } else {
        echo "<script>alert('Error updating location code!');</script>";
    }
}
?>
                                                ...
                                                <td>
                                                    <center>
                                                        <form method="POST" action="">
                                                            <input type="text" name="location_code" value="<?php echo (isset($rows['location_code']) && $rows['location_code'] != 'NULL' && !is_null($rows['location_code'])) ? htmlspecialchars($rows['location_code']) : ''; ?>" placeholder="Enter location code" class="form-control" style="margin-bottom: 10px;">
                                                            <button type="submit" name="update_location" class="btn btn-success">Submit</button>
                                                        </form>
                                                    </center>
                                                </td>
                                            </tr>
